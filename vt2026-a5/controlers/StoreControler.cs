    ...
    internal class storeControler
    {
        ...
        public storeControler(string path)
        {
            storeData = new(
                ...
                CSVparse.GetList(path));
            this.path = path;
            Logger.Log(GetList()); Debug.WriteLine("!");
        }

        internal List<SalesObject> GetList()
        {
        ...
        internal void UpdateInfo(Label nameLbl, Label categoryLbl, Label stockLbl, Label tagsLbl, ListView itemsLw)
        {
        ...
        /// <param name="total"></param>
        internal void Buy(ListView shoppingcart, Label total)
        {
        ...
        internal void ShowTop10(Label label)
        {
            ...
            foreach (int? i in test)
            {
                ...
                for (int j = 0; j < storeData.List.Count; j++)
                {
                    ...
                    {
                        output += (l++)+". "+storeData.List[j].Name + "\n";
                        cont=true;
                    }
                }
                if (cont) { continue; }
