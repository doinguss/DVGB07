...
namespace vt2026_a4
{
    public partial class warehouseFrm : Form
    {
        ...
        public warehouseFrm(string path)
        {
            ...
            syncClock.Tick += new(delegate (object? o, EventArgs e) {  UpdateStock();  });
            syncClock.Start();
        }
        ...
        {
            syncWithToolStripMenuItem.PerformClick();
        }

        private void warehouseFrm_Load(object sender, EventArgs e)
        {
            updateStocksFromDatabaseToolStripMenuItem.PerformClick();
            searchBtn.PerformClick();
        }

        private void updateStocksFromDatabaseToolStripMenuItem_Click(object sender, EventArgs e)
        {
            UpdateStock();
        }
        ...
        private void UpdateStock()
        {
            ...
            catch (Exception ex) { errorlogLbl.Text += "<error> " + ex.Message + "</error>\n"; }
        }

        private void syncWithToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                warehouseControler.save("");

            }
            catch (Exception ex) { errorlogLbl.Text += "<error> " + ex.Message + "</error>\n"; }
        }

        private void showPriceAndAmountGraphsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            new GraphsFrm().Show(); 
        }
    }
